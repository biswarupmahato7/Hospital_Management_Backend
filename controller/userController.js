...
export const patientRegister = catchAsyncErrors (async(req,res,next) =>{
    const{firstName,lastName,email,phone,password,dob,gender,nic,role} = 
    req.body;
...
export const addNewAdmin = catchAsyncErrors(async(req,res,next) =>{
    ...
    const admin = await User.create({firstName,lastName,email,phone,password,dob,gender,nic,
        role: "Admin",
...
export const addDoctor = catchAsyncErrors(async(req,res,next) =>{
    ...
    if(isRegistered){
       return next(
        new ErrorHandler(`${isRegistered} already registered with this email`,
        400)); 
    }
